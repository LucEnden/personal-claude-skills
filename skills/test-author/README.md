...
1. **Bootstraps conventions** (first run only) — scans the project to derive shared helper directory, fixtures directory, test file pattern, runner commands per layer, i18n helper, and a §V group map from `SPEC.md` headings. Asks you to confirm findings, then writes `.claude/test-conventions.md`. Subsequent runs load from that file instantly.
...
Both `test-author` and `test-atom-extract` read from and write to `.claude/test-conventions.md`. Whichever skill runs first on a project bootstraps the file. The other inherits it at no cost.
